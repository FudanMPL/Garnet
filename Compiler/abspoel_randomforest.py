...
class RandomForest:
    """
    Random forest wrapper over `abspoel.TreeTrainer`.

    Notes:
    - Uses per-tree feature sub-sampling (random subspace).
    - Bootstrap sampling is omitted for now.
    - Aggregation is majority vote over `abspoel.label_number` classes.
    """
    ...
    def __init__(self, x=None, y=None, h=None, tree_number=None,
        ...
        self.trees = []  # list of (feature_ids, trainer)
    ...
    def fit(self):
        ...
        for t in range(self.tree_number):
            ...
            trainer = abspoel.TreeTrainer(x_sub, self.y, self.h)
            # Ring32 compile safety: limit fixed-point precision for the underlying
            # gini/newton operations inside abspoel training.
            sfix.set_precision(8, 23)
            cfix.set_precision(8, 23)
            trainer.train()
            self.trees.append((feature_ids, trainer))
        return self

    def _predict_one(self, sample):
        votes = sint.Array(abspoel.label_number)
        votes.assign_all(0)
        for feature_ids, trainer in self.trees:
            sub = Array.create_from([sample[i] for i in feature_ids])
            pred = trainer.predict(sub)
            for lbl in range(abspoel.label_number):
    ...
    def predict(self, x):
        ...
        # Avoid multithread tapes because node-based prediction uses MemValue
        # objects created on the main tape.
        @for_range(n)
        def _(i):
    ...
    def test(self, x, y, set_name="test"):
        print_ln("abspoel randomforest test for %s set", set_name)
