<template>
  ...
  <el-form inline="true" :model="form" :rules="rules" label-width="200px" ref="data3">
  <el-form-item prop = 'baseport' label="端口号">
      <el-input v-model="form.baseport" />
    </el-form-item>
  </el-form>
...
import { getAllProtocol,getAllModel,userCreateLocateTask,userPostData,userFileToTask } from '../../api/locateCompute.js'
...
const rules = {
  ...
  baseport: [
    { required: true, message: '请输入基端口号', trigger: 'change' },
    {
      pattern: /^[0-9]+$/,
      message: '只能输入数字',
      trigger: 'change'
...
const submitTask = async () => {
  //进行校验
  console.log(form.value.protocol)
  if(form.value.pN =='2'&&form.value.protocol == ''){//当用户选择了参与方数量但没有选择协议时自动为其填充
    form.value.protocol = '3'
    console.log(form.value.protocol)
  }else if (form.value.pN == '3'&&form.value.protocol == ''){
  ...
  //提交任务表
  const res = await userCreateTask(form.value)
