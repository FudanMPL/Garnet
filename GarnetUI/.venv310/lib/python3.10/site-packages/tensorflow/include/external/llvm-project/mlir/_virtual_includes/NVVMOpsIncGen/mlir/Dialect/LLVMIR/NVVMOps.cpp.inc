...
SyncWarpOpAdaptor::SyncWarpOpAdaptor(::mlir::ValueRange values, ::mlir::DictionaryAttr attrs, ::mlir::RegionRange regions) : odsOperands(values), odsAttrs(attrs), odsRegions(regions) {  if (odsAttrs)
    odsOpName.emplace("nvvm.bar.warp.sync", odsAttrs.getContext());
}

SyncWarpOpAdaptor::SyncWarpOpAdaptor(SyncWarpOp op) : odsOperands(op->getOperands()), odsAttrs(op->getAttrDictionary()), odsRegions(op->getRegions()), odsOpName(op->getName()) {}

::mlir::ValueRange SyncWarpOpAdaptor::getOperands() {
  return odsOperands;
}

std::pair<unsigned, unsigned> SyncWarpOpAdaptor::getODSOperandIndexAndLength(unsigned index) {
  return {index, 1};
}

::mlir::ValueRange SyncWarpOpAdaptor::getODSOperands(unsigned index) {
  auto valueRange = getODSOperandIndexAndLength(index);
  return {std::next(odsOperands.begin(), valueRange.first),
           std::next(odsOperands.begin(), valueRange.first + valueRange.second)};
}

::mlir::Value SyncWarpOpAdaptor::getMask() {
  return *getODSOperands(0).begin();
}

::mlir::DictionaryAttr SyncWarpOpAdaptor::getAttributes() {
  return odsAttrs;
}
