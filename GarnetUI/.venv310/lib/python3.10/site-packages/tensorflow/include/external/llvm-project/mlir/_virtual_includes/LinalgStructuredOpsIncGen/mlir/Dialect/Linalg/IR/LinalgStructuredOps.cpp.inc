...
BatchReduceMatmulOpAdaptor::BatchReduceMatmulOpAdaptor(::mlir::ValueRange values, ::mlir::DictionaryAttr attrs, ::mlir::RegionRange regions) : odsOperands(values), odsAttrs(attrs), odsRegions(regions) {  if (odsAttrs)
    odsOpName.emplace("linalg.batch_reduce_matmul", odsAttrs.getContext());
}

BatchReduceMatmulOpAdaptor::BatchReduceMatmulOpAdaptor(BatchReduceMatmulOp op) : odsOperands(op->getOperands()), odsAttrs(op->getAttrDictionary()), odsRegions(op->getRegions()), odsOpName(op->getName()) {}

::mlir::ValueRange BatchReduceMatmulOpAdaptor::getOperands() {
  return odsOperands;
}

std::pair<unsigned, unsigned> BatchReduceMatmulOpAdaptor::getODSOperandIndexAndLength(unsigned index) {
  assert(odsAttrs && "missing segment size attribute for op");
...
::mlir::ValueRange BatchReduceMatmulOpAdaptor::getODSOperands(unsigned index) {
  auto valueRange = getODSOperandIndexAndLength(index);
  return {std::next(odsOperands.begin(), valueRange.first),
           std::next(odsOperands.begin(), valueRange.first + valueRange.second)};
}

::mlir::ValueRange BatchReduceMatmulOpAdaptor::getInputs() {
  return getODSOperands(0);
}

::mlir::ValueRange BatchReduceMatmulOpAdaptor::getOutputs() {
  return getODSOperands(1);
}

::mlir::DictionaryAttr BatchReduceMatmulOpAdaptor::getAttributes() {
  return odsAttrs;
}

::mlir::RegionRange BatchReduceMatmulOpAdaptor::getRegions() {
  return odsRegions;
}

::mlir::Region &BatchReduceMatmulOpAdaptor::getRegion() {
  return *odsRegions[0];
}
...
::mlir::Value MapOpAdaptor::getInit() {
...
::mlir::Region &MapOpAdaptor::getMapper() {
  return *odsRegions[0];
}
...
::mlir::ValueRange ReduceOpAdaptor::getInits() {
  return getODSOperands(1);
}
...
::mlir::DenseI64ArrayAttr ReduceOpAdaptor::getDimensionsAttr() {
  assert(odsAttrs && "no attributes when constructing adapter");
...
::llvm::ArrayRef<int64_t> ReduceOpAdaptor::getDimensions() {
...
::mlir::Region &ReduceOpAdaptor::getCombiner() {
  return *odsRegions[0];
}
...
::mlir::LogicalResult ReduceOp::verifyInvariantsImpl() {
  ...
  if (::mlir::failed(__mlir_ods_local_attr_constraint_LinalgStructuredOps9(*this, tblgen_dimensions, "dimensions")))
    return ::mlir::failure();
  {
    unsigned index = 0; (void)index;
    auto valueGroup0 = getODSOperands(0);
