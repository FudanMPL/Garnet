...
#ifndef PROTOCOLS_VSSBEAVER_HPP_
#define PROTOCOLS_VSSBEAVER_HPP_
...
template <class T>
typename T::Protocol VssBeaver<T>::branch()
{
...
template <class T>
void VssBeaver<T>::init(Preprocessing<T> &prep, typename T::MAC_Check &MC)
{
    this->prep = &prep;
    this->MC = &MC;
}
...
template <class T>
void VssBeaver<T>::prepare_mul(const T &x, const T &y, int n)
{
    (void)n;
    triples.push_back({{}});
...
template <class T>
void VssBeaver<T>::start_exchange()
{
    MC->POpen_Begin(opened, shares, P); 
}

template <class T>
void VssBeaver<T>::stop_exchange()
{
...
template <class T>
T VssBeaver<T>::finalize_mul(int n)
{
    (void)n;
