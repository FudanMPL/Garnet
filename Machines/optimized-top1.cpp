...
public:
    ...
    KNN_party_SecKNN(int playerNo):KNN_party_base(playerNo){
        std::cout<<"Entering the optimized's top1 class:"<<std::endl;
    }
    ...
    void top_1_optimized(vector<array<Z2<K>,2>>&shares,int size_of_need_select,bool min_in_last=true);
    void run();
...
public:
    ...
    void top_1(vector<array<Z2<K>,2>>&shares,int size_of_need_select,bool min_in_last=true);
    void top_1_secknn(vector<array<Z2<K>,2>>&shares,int size_of_need_select,bool min_in_last=true);
...
void KNN_party_SecKNN::run()
{
    ...
    for(int idx=0;idx<num_test_data;idx++)
    {
        ...
        {
            top_1_optimized(m_ESD_vec,num_train_data-i,true);//比较排序次数依次是279，278，277，276，275，每次比较排序需要用到的轮次为2（比较一轮+乘法一轮）
        }
...
void KNN_party_SecKNN::top_1_optimized(vector<array<Z2<K>,2>>&shares,int size_now,bool min_in_last)
{
