...
public:
    ...
    KNN_party_optimized(int playerNo):KNN_party_base(playerNo){
        std::cout<<"Entering the KNN with optimized's ESD class:"<<std::endl;
    }
...
void KNN_party_optimized::run()
{
    ...
    for(int idx=0 ; idx<num_test_data ; idx++)
    {
        ...
        // 选择top-k 最小的k个值放到最后面的k个位置
        for(int i=0;i<k_const;i++){
            top_1_secknn(m_ESD_vec,num_train_data-i,true); //比较排序次数依次是279，278，277，276，275，每次比较排序需要用到的轮次为2（比较一轮+乘法一轮）
...
void KNN_party_optimized::top_1_secknn(vector<array<Z2<K>,2>>&shares,int size_of_need_select,bool min_in_last)
{
    for(int i=0;i<size_of_need_select-1;i++)
        SS_scalar(shares, i, size_of_need_select-1, !min_in_last);
}
