...
void fss_generate(uint8_t * r, uint8_t * seed0, uint8_t * seed1, uint8_t * generated_value_cpu, int numbytes, int parallel){
  ...
  for(int i = 0; i < bit_length-1; i++){
    ...
    for(int j = 0; j < 2; j++){
      _copy(cuda_fss_gen->seed[j], cuda_fss_gen->inter_val[j], 0, 0, numbytes);
      _copy(cuda_fss_gen->seed[j], cuda_fss_gen->inter_val[j], 0, numbytes, numbytes);
      _copy(cuda_fss_gen->seed[j], cuda_fss_gen->inter_val[j], 0, 2*numbytes, 1);
      ...
      for(int k = 0; k < 2; k++){
        ...
        _copy(cuda_fss_gen-> inter_val[j], cuda_fss_gen->t[j][0], 0, 0, 1);
        _mod2_t(cuda_fss_gen->t[j][0]);
        _copy(cuda_fss_gen->inter_val[j], cuda_fss_gen->v[j][0], 1, 0, numbytes);
        _copy(cuda_fss_gen->inter_val[j], cuda_fss_gen->s[j][0], numbytes + 1, 0,  numbytes);

        AES_Encrypt<<<num_sm, thrdperblock>>>(cuda_fss_gen->inter_val[j], cuda_key, 176, numbytes, 3);
        _copy(cuda_fss_gen-> inter_val[j], cuda_fss_gen->t[j][1],  0, 0, 1);
        _mod2_t(cuda_fss_gen->t[j][1]);
        _copy(cuda_fss_gen->inter_val[j], cuda_fss_gen->v[j][1], 1, 0, numbytes);
        _copy(cuda_fss_gen->inter_val[j], cuda_fss_gen->s[j][1], 1 + numbytes, 0, numbytes);
      }
    }
    _xor(cuda_fss_gen->s[lose][0], cuda_fss_gen->s[lose][1], cuda_fss_gen->scw, numbytes);
    for(int j = 0; j < 2; j++){
        _copy(cuda_fss_gen->v[lose][j], cuda_fss_gen->convert_seed[j], 0, 0, numbytes);
        AES_Encrypt<<<num_sm, thrdperblock>>>(cuda_fss_gen->seed[j], cuda_key, 176, numbytes, 1);
        _copy(cuda_fss_gen->convert_seed[j], cuda_fss_gen->convert[i], 0, 0, numbytes);
    }
    if(keep){
    ...
    for(int j = 0; j < 2; j++){
        _copy(cuda_fss_gen->v[keep][j], cuda_fss_gen->convert_seed[j], 0, 0, numbytes);
        AES_Encrypt<<<num_sm, thrdperblock>>>(cuda_fss_gen->convert_seed[j], cuda_key, 176, numbytes, 1);
        _copy(cuda_fss_gen->convert_seed[j], cuda_fss_gen->convert[j], 0, 0, numbytes);
    }
    va_genearte_update<<<1,1>>>(cuda_fss_gen, keep, numbytes);
    tcw_pret_generate_update<<<1,1>>>(cuda_fss_gen, keep, numbytes);
    _copy(cuda_fss_gen->scw, generated_value, 0, 2*(numbytes+1)*i, numbytes);
    _copy(cuda_fss_gen->vcw, generated_value, 0, 2*(numbytes+1)*i + numbytes, numbytes); 
    _copy(cuda_fss_gen->tcw[0], generated_value, 0, 2*(numbytes+1)*i + 2 * numbytes, 1);
    _copy(cuda_fss_gen->tcw[1], generated_value, 0, 2*(numbytes+1)*i + 2 * numbytes + 1, 1);
  }
  
  for(int j = 0; j < 2; j++){
      _copy(cuda_fss_gen->seed[j], cuda_fss_gen->convert_seed[j], 0, 0, numbytes);
      AES_Encrypt<<<num_sm, thrdperblock>>>(cuda_fss_gen->convert_seed[j], cuda_key, 176, numbytes, 1);
      _copy(cuda_fss_gen->convert_seed[j], cuda_fss_gen->convert[j], 0, 0, numbytes);
  }
