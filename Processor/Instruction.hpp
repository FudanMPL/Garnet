...
inline void BaseInstruction::parse(istream &s, int inst_pos)
{
  n = 0;
  start.resize(0);
  r[0] = 0;
...
inline void BaseInstruction::parse_operands(istream &s, int pos, int file_pos)
{
  int num_var_args = 0;
  switch (opcode)
  {
  ...
  case PSIALIGN:
    ...
    // cout << "psi align num var: " << num_var_args << endl;
    r[0] = get_int(s);
    r[1] = get_int(s);
  ...
  case PSI:
    ...
    // cout << "num_var_args" << num_var_args << endl;
    get_ints(r, s, num_var_args);
    get_vector(num_var_args, start, s);
    // for (auto x : start)
    //     cout << x << endl;
    // cout << "parse\n";
...
inline bool Instruction::get_offline_data_usage(DataPositions &usage)
{
  switch (opcode)
  {
  case USE:
...
inline int BaseInstruction::get_reg_type() const
{
  switch (opcode & 0x2B0)
  {
  case SECRET_WRITE:
...
inline unsigned BaseInstruction::get_max_reg(int reg_type) const
{
  int skip = 0;
  int offset = 0;
  int size_offset = 0;
  ...
  case PSIALIGN:
    return r[0] + start[0] * (start[1] + start[2]);
  case PSI:
    return r[0] + start[0];
  case MATMULS:
  {
    ...
    {
      unsigned tmp = start[0] + start[i + 3] * start[i + 5];
      res = max(res, tmp);
    }
    return res;
  }
...
inline size_t BaseInstruction::get_mem(RegType reg_type) const
{
...
inline bool BaseInstruction::is_direct_memory_access() const
{
  switch (opcode)
  {
  case LDMS:
...
inline void Instruction::execute(Processor<sint, sgf2n> &Proc) const
{
  ...
  for (int i = 0; i < size; i++)
  {
    ...
    case READFILESHARE:
      ...
      break;
    case PUBINPUT:
      Proc.get_Cp_ref(r[0]) = Proc.template get_input<IntInput<typename sint::clear>>(
                                      Proc.public_input, Proc.public_input_filename, 0)
                                  .items[0];
      break;
    case RAWOUTPUT:
...
#define X(NAME, PRE, CODE) case NAME:
      REGINT_INSTRUCTIONS
#undef X
      throw runtime_error("wrong case statement");
      return;
    }
    if (size > 1)
    {
...
#define X(NAME, PRE, CODE) case NAME:
      CLEAR_GF2N_INSTRUCTIONS
      instruction.execute_clear_gf2n(Proc2.get_C(), Proc.machine.M2.MC, Proc);
      break;
#undef X
#define X(NAME, PRE, CODE) case NAME:
      REGINT_INSTRUCTIONS
      instruction.execute_regint(Proc, Proc.machine.Mi.MC);
      break;
#undef X
#define X(NAME, CODE) \
  case NAME:          \
