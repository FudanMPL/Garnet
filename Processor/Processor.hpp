...
template <class T>
void SubProcessor<T>::psi_align(const vector<typename T::clear> &source, const Instruction &instruction)
{
  auto &dim = instruction.get_start();
  auto res = S.begin() + (instruction.get_r(0));
  ...
  if (!fid.is_open())
  {
    cerr << "Error opening file: " << P.my_num() << endl;
    return;
  }
  idtype id_tmp, l;
...
void SubProcessor<T>::psi(const Instruction &instruction)
{
  ...
  auto res = C.begin() + (instruction.get_r(0));
